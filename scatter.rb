...
def draw_chart(data_series)
  ...
  ## Find max and min values so we know what area to graph
  x_min = nil
  x_max = nil
  y_min = nil
  y_max = nil
  data_series.each do |point|
  ...
  svg = "<svg width='#{svg_x}' height='#{svg_y}' xmlns='http://www.w3.org/2000/svg'>"
    ...
    if y_min <= 0 and y_max >= 0
      svg +=  "<line x1='0' y1='#{svg_y - svg_y_0}' x2='#{svg_x}' y2='#{svg_y - svg_y_0}' style='stroke:black;stroke-width:2' />"
    end
    ...
    data_series.each do |point|
      ...
      svg += "<circle cx='#{x}' cy='#{svg_y - y}' " 
      if point.size >=4
        svg += "r='#{point[3][:rad]}' fill='#{point[3][:color]}' fill-opacity='#{point[3][:alpha]}'>"
      else
        svg += "r='3' fill='grey'>"
      end
      svg += "<title>"
      svg += (point[2] + ": ") if point.size >=3
      svg += "#{sprintf("%0.02f", point[0])}, #{sprintf("%0.02f", point[1])}</title>"
      svg += "</circle>"
      if point.size >=5
        svg += "</a>"
      end
